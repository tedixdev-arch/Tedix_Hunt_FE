...
export function PublicHuntAccessPage() {
  ...
  return <main className="grid min-h-dvh place-items-center bg-[#07110f] px-5 py-8 text-white"><section className="w-full max-w-md rounded-2xl border border-white/10 bg-white/[0.06] p-6 shadow-xl sm:p-8"><Link className="text-sm font-black uppercase tracking-[0.2em]" to="/">TedixHunt</Link>
    ...
    {hunt && <><p className="mt-8 text-xs font-black uppercase tracking-[0.18em] text-emerald-300">Hunt invitation</p><h1 className="mt-3 text-3xl font-black">{hunt.name}</h1><dl className="mt-6 space-y-4 rounded-xl bg-black/20 p-4"><div><dt className="text-xs font-bold uppercase tracking-wide text-slate-400">Code</dt><dd className="mt-1 text-xl font-black tracking-[0.16em]">{hunt.code.toUpperCase()}</dd></div><div><dt className="text-xs font-bold uppercase tracking-wide text-slate-400">Status</dt><dd className="mt-1 font-black text-emerald-300">{accessStatusLabels[hunt.status] ?? 'Hunt unavailable'}</dd></div></dl><p className="mt-6 text-sm leading-6 text-slate-300">Participant joining will be connected in the next phase.</p><button className="mt-5 min-h-11 text-sm font-bold text-slate-300" onClick={() => history.back()} type="button">← Go back</button></>}
  </section></main>;
}
