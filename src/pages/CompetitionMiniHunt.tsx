    ...
    <main className="h-dvh overflow-hidden bg-[#07110f] text-white">
      <div className="mx-auto flex h-full w-full max-w-lg flex-col px-5 py-4">
        <header className="flex min-h-9 items-center justify-between">
          <Link className="text-sm font-black uppercase tracking-[0.2em]" to="/">TedixHunt</Link>
        ...
        {stage === 'personal' && (
          <section className="flex min-h-0 flex-1 flex-col justify-center py-3">
            ...
            <h1 className="mt-1 text-3xl font-black tracking-tight">{checkpoint.title}</h1>
            <div className="mt-3 rounded-xl border border-white/10 bg-white/[0.045] p-3">
              ...
              <div className="mt-3 grid gap-1.5 font-mono text-base font-black" aria-label="Input and output examples">
            ...
            <form className="mt-3" onSubmit={submitPersonal}>
              ...
              <button className="mt-2 min-h-12 w-full rounded-xl bg-emerald-400 px-5 font-black text-slate-950" type="submit">Check answer</button>
            </form>
            ...
            {personalCorrect && feedback && <div className="mt-2 grid grid-cols-[1fr_auto] gap-2"><p className="flex items-center rounded-lg border border-emerald-300/30 bg-emerald-300/[0.07] px-3 font-mono text-sm font-black text-emerald-200">{checkpoint.personalMask}</p><button className="min-h-12 rounded-lg border border-emerald-300/40 px-4 text-sm font-bold text-emerald-200" onClick={() => { setFeedback(''); setStage('team-info') }}>Next →</button></div>}
          </section>
        )}
        ...
        {stage === 'team-info' && (
          <section className="flex flex-1 flex-col justify-center py-8">
            ...
            <h1 className="mt-3 text-4xl font-black tracking-tight">Every Hunter found different letters</h1>
            <div className="mt-7 space-y-3 rounded-2xl border border-cyan-300/20 bg-cyan-300/[0.07] p-5">{checkpoint.publicContributions.map((item) => <p className={`rounded-lg px-4 py-3 font-mono text-sm font-bold ${item.startsWith('You:') ? 'bg-emerald-300/15 text-emerald-200' : 'bg-white/[0.055]'}`} key={item}>{item}</p>)}</div>
            ...
            <button className="mt-7 min-h-16 rounded-xl bg-emerald-400 px-6 font-black text-slate-950" onClick={() => setStage('team-challenge')}>Open the team challenge</button>
          </section>
        )}
        ...
        {stage === 'team-challenge' && (
          <section className="flex flex-1 flex-col justify-center py-8">
            ...
            <h1 className="mt-3 text-4xl font-black tracking-tight">Combine the discoveries</h1>
            <p className="mt-5 text-slate-300">{checkpoint.teamPrompt}</p>
            <p className="mt-5 rounded-xl border border-white/10 bg-white/[0.05] px-4 py-4 text-center font-mono font-black tracking-[0.12em]">{checkpoint.teamDisplay}</p>
            <form className="mt-6" onSubmit={submitTeam}>
              ...
              <button className="mt-4 min-h-14 w-full rounded-xl bg-emerald-400 px-5 font-black text-slate-950" type="submit">Submit team answer</button>
            </form>
        ...
        {stage === 'direction' && (
          <section className="flex flex-1 flex-col justify-center py-8 text-center">
            ...
            <div className="mx-auto mt-6 grid h-28 w-28 place-items-center rounded-full border border-emerald-300/40 bg-emerald-300/[0.08] text-4xl font-black text-emerald-200">{checkpoint.direction}</div>
            <h1 className="mt-6 text-4xl font-black tracking-tight">{checkpoint.nextLocation}</h1>
            ...
            <button className="mt-8 min-h-16 rounded-xl bg-emerald-400 px-6 font-black text-slate-950" onClick={() => navigate('/discover/passport')}>Complete the mini-hunt</button>
          </section>
        )}
      </div>
      ...
      {calculatorOpen && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/75 p-3 sm:items-center" role="dialog" aria-modal="true" aria-labelledby="mini-calculator-title">
          <section className="w-full max-w-sm rounded-2xl bg-slate-100 p-4 text-slate-950 shadow-2xl">
            <div className="flex items-center justify-between"><h2 className="text-lg font-black" id="mini-calculator-title">Calculator</h2><button className="min-h-11 px-3 font-bold text-slate-500" onClick={() => setCalculatorOpen(false)} aria-label="Close calculator">×</button></div>
            <output className="mt-2 flex min-h-14 items-center justify-end rounded-xl bg-slate-900 px-4 text-2xl font-black text-white" aria-live="polite">{personalAnswer || '0'}</output>
            <div className="mt-3 grid grid-cols-3 gap-2">{['1','2','3','4','5','6','7','8','9'].map((digit) => <button className="min-h-12 rounded-lg bg-white font-black shadow-sm" key={digit} onClick={() => { setPersonalAnswer(`${personalAnswer}${digit}`); setFeedback('') }}>{digit}</button>)}<button className="min-h-12 rounded-lg bg-red-100 font-black text-red-700" onClick={() => setPersonalAnswer('')}>Clear</button><button className="min-h-12 rounded-lg bg-white font-black shadow-sm" onClick={() => setPersonalAnswer(`${personalAnswer}0`)}>0</button><button className="min-h-12 rounded-lg bg-white font-black shadow-sm" onClick={() => setPersonalAnswer(personalAnswer.slice(0, -1))}>⌫</button></div>
            <button className="mt-3 min-h-12 w-full rounded-xl bg-emerald-500 font-black text-slate-950" onClick={() => setCalculatorOpen(false)}>Use answer</button>
          </section>
        </div>
      )}
    </main>
