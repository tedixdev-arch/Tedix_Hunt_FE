...
function CheckpointMapEditor({ onVerifiedChange }: { onVerifiedChange: (count: number) => void }) {
  ...
  return (
    <section className="mt-6 border-t border-slate-100 pt-6" aria-labelledby="checkpoint-map-title">
      <div className="flex flex-wrap items-end justify-between gap-3"><div><h3 className="text-lg font-bold" id="checkpoint-map-title">Check every checkpoint on the map</h3><p className="mt-1 text-sm text-slate-500">Select a pin, click the map to fix its position, then verify it. Participants will be marked in range within 30 metres.</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${checked === 7 ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-900'}`}>{checked} of 7 verified</span></div>
      <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_280px]">
        <div aria-label="Checkpoint location map" className="relative min-h-[360px] cursor-crosshair overflow-hidden rounded-2xl border border-slate-300 bg-[#e7eee8]" onClick={moveSelected} role="application">
          ...
          {positions.map(point => <button aria-label={`Checkpoint ${point.id}: ${point.name}${verified.has(point.id) ? ', verified' : ''}`} className={`absolute grid h-10 w-10 -translate-x-1/2 -translate-y-full place-items-center rounded-full border-4 text-sm font-bold shadow-lg transition ${verified.has(point.id) ? 'border-white bg-emerald-500 text-white' : selected === point.id ? 'border-emerald-200 bg-slate-950 text-white ring-4 ring-emerald-400/40' : 'border-white bg-amber-400 text-slate-950'}`} key={point.id} onClick={event => { event.stopPropagation(); setSelected(point.id) }} style={{left:`${point.x}%`,top:`${point.y}%`}} type="button">{verified.has(point.id) ? '✓' : point.id}</button>)}
        </div>
        <div className="space-y-2" aria-label="Checkpoint verification list">{positions.map(point => <div className={`rounded-xl border p-3 ${selected === point.id ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200'}`} key={point.id}><button className="w-full text-left" onClick={() => setSelected(point.id)} type="button"><span className="text-xs font-semibold uppercase tracking-wide text-slate-400">Checkpoint {point.id}</span><strong className="mt-1 block text-sm">{point.name}</strong></button><button className={`mt-3 min-h-10 w-full rounded-lg text-xs font-semibold ${verified.has(point.id) ? 'border border-slate-300 bg-white text-slate-700' : 'bg-emerald-500 text-slate-950'}`} onClick={() => toggleVerified(point.id)} type="button">{verified.has(point.id) ? 'Fix position again' : 'Position is correct'}</button></div>)}</div>
      </div>
    </section>
  )
}
...
function StudentPreview({ featureId, name, example }: { featureId: string; name: string; example: string }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-700 bg-[#061812] text-white shadow-lg">
      <div className="flex items-center justify-between border-b border-white/10 px-4 py-3 text-[10px] font-bold uppercase tracking-[0.16em]"><span>TedixHunt</span><span className="text-emerald-300">Preview</span></div>
      <div className="p-4"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-emerald-300">{featureId.replace('-', ' ')}</p><h3 className="mt-2 text-xl font-bold">{name}</h3><p className="mt-3 text-sm leading-6 text-slate-200">{example}</p><div className="mt-5 rounded-lg bg-emerald-400 px-3 py-2 text-center text-xs font-semibold text-slate-950">Student view</div></div>
    </div>
  )
}
...
function GeneralSetup({ independent, initialSettings, initialProgress, organizations, selectedOrganizationId, templates, templatesLoading, templatesError, huntOptions, optionsLoading, optionsError, savedTemplateKey, savedTemplateSnapshot, onRetryTemplates, onRetryOptions, onOrganizationChange, onComplete, onFormatChange, onTeamSizeChange, onSave }: {
  ...
  if (collapsed) return (
    <section className="mt-7 rounded-2xl border border-emerald-300 bg-white p-5 shadow-sm" aria-labelledby="general-title">
      ...
      <div className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-emerald-800">Mission</p><p className="mt-2 font-bold">{settings.theme}</p><p className="mt-1 text-sm text-emerald-950">{settings.mission}</p></div>
    </section>
  )
  ...
  return (
    <section className="mt-7 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" aria-labelledby="general-title">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-700">Block 1</p><h2 className="mt-2 text-2xl font-bold" id="general-title">General Setup</h2><p className="mt-1 text-sm text-slate-500">Define the event before building its participant experience.</p></div>
        <div className="min-w-48"><div className="flex justify-between text-xs font-semibold"><span>{completed.size} of 4 complete</span><span>{progress}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full bg-emerald-500" style={{ width: `${progress}%` }} /></div></div>
      </div>
      <div className="mt-6 grid gap-5 lg:grid-cols-[240px_1fr]">
        <nav className="space-y-2" aria-label="General Setup sections">{generalSteps.map((step, index) => <button className={`flex min-h-12 w-full items-center justify-between rounded-xl border px-4 text-left text-sm font-bold ${active === index ? 'border-emerald-500 bg-emerald-50 text-emerald-900' : 'border-slate-200 bg-slate-50'}`} key={step} onClick={() => { setActive(index); setSaveError('') }} type="button"><span>{index + 1}. {step}</span>{completed.has(index) && <span className="text-emerald-600">✓</span>}</button>)}</nav>
        <article className="rounded-xl border border-slate-200 p-5">
          <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">General {active + 1} of 4</p><h3 className="mt-2 text-xl font-bold">{generalSteps[active]}</h3>
          <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_240px]"><div className="grid gap-4 sm:grid-cols-2">
            ...
            {(active === 1 || active === 2) && !independent && optionsError && <div className="sm:col-span-2 rounded-xl bg-rose-50 p-4"><p className="text-sm font-semibold text-rose-700" role="alert">We couldn't load the supported Hunt options.</p><button className="mt-3 min-h-10 rounded-lg border border-slate-300 bg-white px-4 text-sm font-bold" onClick={onRetryOptions} type="button">Retry</button></div>}
            ...
            {active === 3 && (independent ? <><label className="text-sm font-bold">Hunt theme<select className={controlClass} value={settings.theme} onChange={e=>{const theme=e.target.value;const mission=templatesByTheme[theme][0];setSettings({...settings,theme,mission})}}>{Object.keys(templatesByTheme).map(theme=><option key={theme}>{theme}</option>)}</select></label><label className="text-sm font-bold">Hunt template<select className={controlClass} value={settings.mission} onChange={e=>setSettings({...settings,mission:e.target.value})}>{templatesByTheme[settings.theme].map(mission=><option key={mission}>{mission}</option>)}</select></label></> : <>
              ...
              {templatesError && <div className="sm:col-span-2 rounded-xl bg-rose-50 p-4"><p className="text-sm font-semibold text-rose-700" role="alert">We couldn't load Hunt templates.</p><button className="mt-3 min-h-10 rounded-lg border border-slate-300 bg-white px-4 text-sm font-bold" onClick={onRetryTemplates} type="button">Retry</button></div>}
              {!templatesLoading && !templatesError && missingSavedTemplate && savedTemplateSnapshot && <div className="sm:col-span-2 rounded-xl border border-amber-200 bg-amber-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-amber-800">Saved template · no longer available</p><p className="mt-2 font-bold">{savedTemplateSnapshot.displayName}</p><p className="mt-1 text-sm">{savedTemplateSnapshot.theme} · Version {savedTemplateSnapshot.version}</p></div>}
            ...
            {active === 3 && <div className="sm:col-span-2 rounded-xl bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Template includes</p><p className="mt-2 text-sm text-slate-700">Approved challenges, checkpoint navigation and the final mission.</p><div className="mt-4 border-t border-slate-200 pt-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Hunt mission</p><p className="mt-2 text-sm font-bold text-slate-800">Restore six linked relay points, trace the signal to its source and restart the final transmitter together.</p></div></div>}
          </div><aside className="rounded-xl bg-[#061812] p-4 text-white"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-emerald-300">Setup preview</p><h4 className="mt-3 font-bold">{generalSteps[active]}</h4><p className="mt-3 whitespace-pre-line text-sm leading-6 text-slate-200">{setupPreview}</p></aside></div>
          {saveError && <p className="mt-4 rounded-lg bg-rose-50 px-3 py-2 text-sm font-semibold text-rose-700" role="alert">{saveError}</p>}
          <div className="mt-6 flex justify-between border-t border-slate-100 pt-5"><button className="min-h-12 rounded-xl border border-slate-300 px-5 font-bold disabled:opacity-40" disabled={active === 0 || saving} onClick={() => setActive(active - 1)} type="button">Previous</button><button className="min-h-12 rounded-xl bg-emerald-500 px-6 font-bold disabled:cursor-wait disabled:opacity-60" disabled={saving || (!independent && (active === 1 || active === 2) && !huntOptions)} onClick={() => void saveGeneral()} type="button">{saving ? 'Saving…' : active === 3 ? 'Complete General Setup' : 'Save & continue'}</button></div>
        </article>
      </div>
    </section>
  )
...
function CustomChallengeEditor({ value, onChange }: { value: CustomChallenge; onChange: (next: CustomChallenge) => void }) {
  ...
  return <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2">{answers.map((answer,index)=><label className="text-xs font-semibold text-slate-600" key={labels[index]}>{labels[index]}<input className={controlClass} value={answer} onChange={e=>{const next=[...answers];next[index]=e.target.value;onChange({...value,answers:next})}}/></label>)}</div>
  </div>
}
...
function RewardsEditor({huntId,huntStatus,teamSize,onLoaded}:{huntId:string;huntStatus:Hunt['status'];teamSize:number;onLoaded:(rewards:HuntRewards)=>void}) {
  ...
  if(loadError)return <div className="mt-5 rounded-xl bg-rose-50 p-4"><p className="text-sm font-semibold text-rose-700" role="alert">{loadError}</p><button className="mt-3 min-h-10 rounded-lg border bg-white px-4 text-sm font-bold" onClick={()=>void load()} type="button">Retry</button></div>
  ...
  return <div className="mt-5 space-y-6">{!editable&&<p className="rounded-xl bg-amber-50 p-4 text-sm font-semibold text-amber-950">Rewards are locked after the Hunt is published.</p>}{actionError&&<p className="rounded-xl bg-rose-50 p-4 text-sm font-semibold text-rose-700" role="alert">{actionError}</p>}
    <section className="rounded-xl border border-slate-200 p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-bold">Leaderboard Rewards</h3><p className="mt-1 text-sm text-slate-500">Add as many allocations as needed. Saved rewards stay compact.</p></div>{editable&&<button className="min-h-11 rounded-xl bg-emerald-500 px-4 text-sm font-bold disabled:bg-slate-300" disabled={rewardEditorOpen} onClick={addReward} type="button">+ Add reward</button>}</div>
    {rewards.length===0?<div className="mt-5 rounded-xl bg-slate-50 p-5 text-center text-sm text-slate-500">No leaderboard rewards allocated yet.</div>:<div className="mt-5 space-y-3">{[...rewards].sort((a,b)=>a.place-b.place).map((reward,index)=>reward.saved?<article className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4" key={reward.id}><div><span className="text-xs font-semibold uppercase tracking-wide text-emerald-700">{ordinal(reward.place)}</span><h4 className="mt-1 font-bold">{reward.provider==='tedix_inventory'&&reward.kind==='physical'?'Tedix Mystery Physical Reward':reward.provider==='tedix_inventory'?label(options.virtualCategories,reward.category):reward.name}</h4><p className="mt-1 text-xs text-slate-500">{label(options.providers,reward.provider)} · {label(options.kinds,reward.kind)} · Quantity {reward.quantity}</p></div>{editable&&<div className="flex gap-3"><button className="text-sm font-bold text-emerald-800 disabled:opacity-40" disabled={rewardEditorOpen||busy(reward.id)} onClick={()=>updateReward(reward.id,{...reward,saved:false})} type="button">Edit</button><button className="text-sm font-bold text-rose-700 disabled:opacity-40" disabled={busy(reward.id)} onClick={()=>removeLeaderboard(reward)} type="button">{busy(reward.id)?'Removing…':'Remove'}</button></div>}</article>:<article className="rounded-xl border-2 border-emerald-300 bg-slate-50 p-4" key={reward.id}><div className="flex items-center justify-between"><span className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Reward allocation {index+1}</span><button className="text-xs font-semibold text-slate-500" disabled={busy(reward.id)} onClick={()=>reward.persisted?void load():removeLeaderboard(reward)} type="button">Cancel</button></div><label className="mt-4 block text-sm font-bold">Select place<select className={controlClass} value={reward.place} onChange={e=>updateReward(reward.id,{...reward,place:Number(e.target.value)})}>{Array.from({length:50},(_,place)=><option key={place} value={place+1}>{ordinal(place+1)}</option>)}</select></label><RewardFields options={options} reward={reward} onChange={next=>updateReward(reward.id,next as RewardDraft)}/>{!validReward(reward)&&<p className="mt-3 text-xs font-semibold text-rose-700">Add a valid quantity and all required reward details.</p>}<button className="mt-4 min-h-11 rounded-xl border border-emerald-500 bg-white px-4 text-sm font-bold text-emerald-800 disabled:opacity-40" disabled={!validReward(reward)||busy(reward.id)} onClick={()=>saveLeaderboard(reward)} type="button">{busy(reward.id)?'Saving…':'Save allocation'}</button></article>)}</div>}</section>
    <section className="rounded-xl border border-slate-200 p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-bold">Special Awards</h3><p className="mt-1 text-sm text-slate-500">Choose a predefined metric. Rules cannot be edited by the Organizer.</p></div>{editable&&<button className="min-h-11 rounded-xl border border-emerald-500 bg-white px-4 text-sm font-bold text-emerald-800 disabled:opacity-40" disabled={specialAwards.length===options.specialAwardDefinitions.length||specialEditorOpen} onClick={addSpecial} type="button">+ Add special award</button>}</div><div className="mt-4 rounded-xl bg-amber-50 p-4 text-sm leading-6 text-amber-950"><strong>Physical-prize eligibility</strong><p>Physical prizes go to the highest-ranked eligible team or participant who has not already received one. Virtual rewards do not affect eligibility.</p></div>
    {specialAwards.length===0?<div className="mt-5 rounded-xl bg-slate-50 p-5 text-center text-sm text-slate-500">No special awards selected.</div>:<div className="mt-5 space-y-3">{specialAwards.map((award,index)=>{const definition=options.specialAwardDefinitions.find(item=>item.key===award.definitionKey);if(!definition)return null;return award.saved?<article className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4" key={award.id}><div><span className="text-xs font-semibold uppercase tracking-wide text-emerald-700">{definition.scope} Special Award</span><h4 className="mt-1 font-bold">{definition.name}</h4><p className="mt-1 max-w-xl text-xs text-slate-500">{definition.rule}</p><p className="mt-1 text-xs text-slate-500">Quantity {award.quantity}</p></div>{editable&&<div className="flex gap-3"><button className="text-sm font-bold text-emerald-800" disabled={specialEditorOpen||busy(award.id)} onClick={()=>updateSpecial(award.id,{...award,saved:false})} type="button">Edit</button><button className="text-sm font-bold text-rose-700" disabled={busy(award.id)} onClick={()=>removeSpecial(award)} type="button">{busy(award.id)?'Removing…':'Remove'}</button></div>}</article>:<article className="rounded-xl border-2 border-emerald-300 bg-slate-50 p-4" key={award.id}><div className="flex items-center justify-between"><span className="text-xs font-semibold uppercase tracking-wide text-emerald-700">{definition.scope} award {index+1}</span><button className="text-xs font-semibold text-slate-500" disabled={busy(award.id)} onClick={()=>award.persisted?void load():removeSpecial(award)} type="button">Cancel</button></div><label className="mt-4 block text-sm font-bold">Predefined award<select className={controlClass} value={award.definitionKey} onChange={e=>updateSpecial(award.id,{...award,definitionKey:e.target.value})}>{options.specialAwardDefinitions.map(item=><option disabled={specialAwards.some(existing=>existing.id!==award.id&&existing.definitionKey===item.key)} key={item.key} value={item.key}>{item.scope} · {item.name}</option>)}</select></label><div className="mt-4 rounded-xl border bg-white p-4"><p className="text-xs font-semibold uppercase text-slate-400">Winning rule · read only</p><p className="mt-2 font-bold">{definition.rule}</p><p className="mt-2 text-sm text-slate-600">{definition.description}</p><p className="mt-3 text-xs font-semibold text-slate-500">Eligibility: {definition.eligibility}</p></div><RewardFields options={options} reward={award} onChange={next=>updateSpecial(award.id,next as SpecialAwardDraft)}/>{!validReward(award)&&<p className="mt-3 text-xs font-semibold text-rose-700">Add a valid quantity and all required reward details.</p>}<button className="mt-4 min-h-11 rounded-xl border border-emerald-500 bg-white px-4 text-sm font-bold text-emerald-800 disabled:opacity-40" disabled={!validReward(award)||busy(award.id)} onClick={()=>saveSpecial(award)} type="button">{busy(award.id)?'Saving…':'Save special award'}</button></article>})}</div>}</section>
  </div>
}
export function CustomHuntEditorPage() {
  ...
  if (sharing) return <main className="h-dvh overflow-y-auto bg-slate-100 text-slate-950"><OrganizerHeader showProfile/><div className="mx-auto max-w-4xl px-5 py-8 sm:px-8"><button className="text-sm font-bold text-slate-500" onClick={()=>{setSharing(false);setReviewing(true)}} type="button">← Back to Review Hunt</button><p className="mt-7 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Block 4</p><h1 className="mt-2 text-4xl font-bold">Create link &amp; share</h1><p className="mt-3 text-slate-600">Your Hunt is ready. Give participants one clear way to join.</p><section className="mt-7 rounded-2xl border border-emerald-300 bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Participant access</p><h2 className="mt-2 text-2xl font-bold">Signal: Cluj Napoca</h2><p className="mt-2 text-sm text-slate-600">Use the link, Hunt code or QR code below.</p></div><span className="rounded-full bg-emerald-100 px-3 py-2 text-xs font-semibold text-emerald-800">Hunt created</span></div><div className="mt-6 grid gap-5 sm:grid-cols-[1fr_150px]"><div><label className="text-sm font-bold">Participant link<input aria-label="Hunt invitation link" className={controlClass} readOnly value="https://tedixhunt.app/join/SIGNAL26"/></label><div className="mt-4 rounded-xl bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Hunt code</p><p className="mt-2 text-2xl font-bold tracking-[0.18em]">SIGNAL26</p></div><div className="mt-4 flex flex-col gap-2 sm:flex-row"><button className="min-h-12 flex-1 rounded-xl border border-emerald-400 bg-white px-4 font-bold" onClick={async()=>{await navigator.clipboard?.writeText('https://tedixhunt.app/join/SIGNAL26');setLinkMessage('Link copied')}} type="button">Copy link</button><button className="min-h-12 flex-1 rounded-xl bg-slate-950 px-4 font-bold text-white" onClick={shareHunt} type="button">Share</button></div></div><div className="grid aspect-square place-items-center self-start rounded-xl border-4 border-slate-950 bg-white text-center text-sm font-bold">QR<br/>SIGNAL26</div></div>{linkMessage&&<p className="mt-4 text-sm font-bold text-emerald-800" role="status">✓ {linkMessage}</p>}<div className="mt-6 flex justify-end border-t border-slate-100 pt-5"><Link className="inline-flex min-h-12 items-center rounded-xl bg-emerald-500 px-6 font-bold" to="/organizer">Finish and open My Hunts</Link></div></section></div></main>

  if (reviewing && independent) return <main className="h-dvh overflow-y-auto bg-slate-100 text-slate-950"><OrganizerHeader showProfile/><div className="mx-auto max-w-4xl px-5 py-8 sm:px-8"><button className="text-sm font-bold text-slate-500" onClick={()=>setReviewing(false)} type="button">← Back to Hunt Features</button><p className="mt-7 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Block 3</p><h1 className="mt-2 text-4xl font-bold">Review Hunt</h1><p className="mt-3 text-slate-600">Confirm the setup before creating participant access.</p><section className="mt-7 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Independent Hunt</p><h2 className="mt-2 text-2xl font-bold">Signal: Cluj Napoca</h2><p className="mt-2 text-sm text-slate-600">7 checkpoints · {huntFormat} · Creator-verified route</p></div><span className="rounded-full bg-emerald-100 px-3 py-2 text-xs font-semibold text-emerald-800">Ready to create</span></div><div className="mt-5 rounded-xl bg-amber-50 p-4 text-sm leading-6 text-amber-950"><strong>Independent Hunt</strong><p className="mt-1">This is an unofficial activity. Use the verified route and organize it safely. Rewards are unavailable.</p></div><div className="mt-5 grid gap-3 sm:grid-cols-3">{[['General Setup','Complete'],['Hunt Features',`${completedRequired}/${requiredFeatures.length} complete`],['Route','Verified']].map(([label,value])=><div className="rounded-xl bg-slate-50 p-4" key={label}><p className="text-xs font-semibold text-slate-500">{label}</p><p className="mt-1 font-bold">{value}</p></div>)}</div><div className="mt-6 flex flex-col-reverse gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:justify-between"><button className="min-h-12 rounded-xl border border-slate-300 px-5 font-bold" onClick={()=>setReviewing(false)} type="button">Edit Hunt</button><button className="min-h-12 rounded-xl bg-emerald-500 px-6 font-bold" onClick={()=>{setReviewing(false);setSharing(true)}} type="button">Create Hunt &amp; continue</button></div></section></div></main>
  ...
  if (reviewing && persistedHunt) {
    ...
    if (persistedHunt.status === 'published') {
      ...
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Registered Hunt</p><h1 className="mt-2 text-4xl font-bold">Hunt published</h1>
      <section className="mt-7 rounded-2xl border border-emerald-300 bg-white p-5 shadow-sm sm:p-6"><h2 className="text-2xl font-bold">{persistedHunt.name}</h2><dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2"><div><dt className="text-slate-500">Status</dt><dd className="font-bold">Published</dd></div><div><dt className="text-slate-500">Template</dt><dd className="font-bold">{snapshot?.displayName || 'Not saved'}</dd></div><div><dt className="text-slate-500">Date</dt><dd className="font-bold">{formatDate(persistedHunt.startDate)} · {formatTime(persistedHunt.startTime)}</dd></div><div><dt className="text-slate-500">Location</dt><dd className="font-bold">{formatLocation(persistedHunt)}</dd></div></dl>
        <div className="mt-6 rounded-xl bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Participant access</p>{persistedHunt.accessCode ? <div className="mt-4"><p className="text-sm font-bold text-slate-600">Hunt code</p><p className="mt-1 text-2xl font-bold tracking-[0.18em]">{persistedHunt.accessCode}</p><label className="mt-4 block text-sm font-bold">Participant link<input aria-label="Hunt invitation link" className={controlClass} readOnly value={link}/></label><div className="mt-4 flex flex-col gap-2 sm:flex-row"><button className="min-h-12 flex-1 rounded-xl border border-emerald-400 bg-white px-4 font-bold" onClick={() => void copyRegisteredLink(link)} type="button">Copy link</button><button className="min-h-12 flex-1 rounded-xl bg-slate-950 px-4 font-bold text-white" onClick={() => void shareRegisteredLink(link, persistedHunt.name)} type="button">Share</button></div><p className="mt-3 text-xs text-slate-500">QR code can be added later.</p>{linkMessage && <p className="mt-3 text-sm font-bold text-emerald-800" role="status">{linkMessage}</p>}</div> : <button className="mt-4 min-h-12 rounded-xl bg-emerald-500 px-5 font-bold disabled:bg-slate-300" disabled={creatingAccess} onClick={() => void createParticipantAccess()} type="button">{creatingAccess ? 'Creating participant access…' : 'Create participant access'}</button>}{accessError && <p className="mt-4 rounded-lg bg-rose-50 p-3 text-sm font-semibold text-rose-700" role="alert">{accessError}</p>}</div>
        <div className="mt-6 flex justify-end border-t border-slate-100 pt-5"><Link className="inline-flex min-h-12 items-center rounded-xl bg-emerald-500 px-6 font-bold" to="/organizer">Back to My Hunts</Link></div></section>
    </div></main>
    }
    return <main className="h-dvh overflow-y-auto bg-slate-100 text-slate-950"><OrganizerHeader showProfile/><div className="mx-auto max-w-4xl px-5 py-8 sm:px-8">
      ...
      <p className="mt-7 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Block 3</p><h1 className="mt-2 text-4xl font-bold">Review Hunt</h1><p className="mt-3 text-slate-600">Review the latest settings saved for this Hunt.</p>
      <section className="mt-7 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Registered Hunt</p><h2 className="mt-2 text-2xl font-bold">{persistedHunt.name || 'Unnamed Hunt'}</h2><div className="mt-2 flex flex-wrap gap-2"><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">{statusLabels[persistedHunt.status]}</span><span className={`rounded-full px-3 py-1 text-xs font-semibold ${review.ready ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-900'}`}>{review.ready ? 'Ready for publishing' : 'Needs attention'}</span></div></div></div>
        <div className="mt-6 grid gap-5 lg:grid-cols-3">
          <section className="rounded-xl bg-slate-50 p-4"><h3 className="font-bold">Hunt</h3><dl className="mt-3 space-y-2 text-sm"><div><dt className="text-slate-500">Location</dt><dd className="font-bold">{formatLocation(persistedHunt)}</dd></div><div><dt className="text-slate-500">Date &amp; time</dt><dd className="font-bold">{formatDate(persistedHunt.startDate)} · {formatTime(persistedHunt.startTime)}</dd></div><div><dt className="text-slate-500">Timezone</dt><dd className="font-bold">{persistedHunt.timezone || 'Not saved'}</dd></div><div><dt className="text-slate-500">Duration</dt><dd className="font-bold">{persistedHunt.durationMinutes ? `${persistedHunt.durationMinutes} minutes` : 'Not saved'}</dd></div><div><dt className="text-slate-500">Capacity</dt><dd className="font-bold">{persistedHunt.capacity ? `${persistedHunt.capacity} participants` : 'Not saved'}</dd></div><div><dt className="text-slate-500">Local contact</dt><dd className="font-bold">{persistedHunt.contactName || 'Not saved'}</dd></div></dl></section>
          <section className="rounded-xl bg-slate-50 p-4"><h3 className="font-bold">Template</h3><dl className="mt-3 space-y-2 text-sm"><div><dt className="text-slate-500">Name</dt><dd className="font-bold">{snapshot?.displayName || 'Not saved'}</dd></div><div><dt className="text-slate-500">Theme</dt><dd className="font-bold">{snapshot?.theme || 'Not saved'}</dd></div><div><dt className="text-slate-500">Version</dt><dd className="font-bold">{persistedHunt.templateVersion ?? 'Not saved'}</dd></div><div><dt className="text-slate-500">Route</dt><dd className="font-bold">{snapshot ? `${snapshot.checkpointNames.length} checkpoints` : 'Not saved'}</dd></div></dl>{snapshot && snapshot.checkpointNames.length > 0 && <details className="mt-3 text-sm"><summary className="cursor-pointer font-bold">Checkpoint names</summary><ol className="mt-2 list-decimal space-y-1 pl-5 text-slate-600">{snapshot.checkpointNames.map(name => <li key={name}>{name}</li>)}</ol></details>}</section>
          <section className="rounded-xl bg-slate-50 p-4"><h3 className="font-bold">Pilot configuration</h3><dl className="mt-3 space-y-2 text-sm">{[['Format',labels.format],['Team size',labels.teamSize],['Access',labels.accessMode],['Difficulty',labels.difficulty],['Checkpoint order',labels.checkpointOrder]].map(([term,value])=><div key={term}><dt className="text-slate-500">{term}</dt><dd className="font-bold">{value}</dd></div>)}</dl></section>
        </div>
        <div className="mt-5 grid gap-4 sm:grid-cols-2"><div className="rounded-xl border border-slate-200 p-4"><p className="text-xs font-semibold text-slate-500">Hunt Features</p><p className="mt-1 font-bold">Prototype configuration</p><p className="mt-1 text-sm text-slate-600">Other feature persistence comes later and does not affect saved-draft readiness.</p></div><div className="rounded-xl border border-slate-200 p-4"><p className="text-xs font-semibold text-slate-500">Rewards</p>{loadedRewards && (loadedRewards.leaderboard.length||loadedRewards.specialAwards.length)?<><p className="mt-1 font-bold">Leaderboard rewards: {loadedRewards.leaderboard.length}</p><p className="mt-1 text-sm font-bold">Special awards: {loadedRewards.specialAwards.length}</p></>:<p className="mt-1 font-bold">No rewards configured</p>}<p className="mt-1 text-xs text-slate-500">Rewards are optional and do not affect readiness.</p></div></div>
        {publishError && <p className="mt-5 rounded-xl bg-rose-50 p-4 text-sm font-semibold text-rose-700" role="alert">{publishError}</p>}
        {uniqueIssues.length > 0 && <section className="mt-5 rounded-xl bg-amber-50 p-4" aria-labelledby="review-issues"><h3 className="font-bold text-amber-950" id="review-issues">{publishIssues.length ? 'Publishing issues' : 'Required settings'}</h3><ul className="mt-3 space-y-2">{uniqueIssues.map(issue => <li className="flex flex-wrap items-center justify-between gap-3 text-sm" key={`${issue.generalSection}-${issue.field}-${issue.message}`}><span>{issue.message}</span><button className="rounded-lg border border-amber-300 bg-white px-3 py-2 font-bold" onClick={()=>editGeneralSection(issue.generalSection)} type="button">Fix in General {issue.generalSection}</button></li>)}</ul></section>}
        <div className="mt-6 flex flex-col-reverse gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:justify-between">{persistedHunt.status === 'draft' && <button className="min-h-12 rounded-xl border border-slate-300 px-5 font-bold" onClick={()=>setReviewing(false)} type="button">Edit Hunt</button>}<div className="text-right"><button className="min-h-12 rounded-xl bg-emerald-500 px-6 font-bold disabled:bg-slate-300" disabled={publishing || persistedHunt.status !== 'draft' || !review.ready || publishIssues.length > 0} onClick={() => void publishHunt()} type="button">{publishing ? 'Publishing…' : persistedHunt.status !== 'draft' ? statusLabels[persistedHunt.status] : review.ready && !publishIssues.length ? 'Publish Hunt' : 'Complete required settings'}</button></div></div>
      </section>
    </div></main>
  }
  ...
  return (
    <main className="h-dvh overflow-y-auto bg-slate-100 text-slate-950">
      ...
      <div className="mx-auto max-w-6xl px-5 py-7 sm:px-8 sm:py-10">
        ...
        <div className="mt-6"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">{independent?'Independent Organizer':'Registered Organizer'}</p><h1 className="mt-2 text-4xl font-bold tracking-tight">Set the Hunt</h1><p className="mt-3 max-w-3xl text-slate-600">See the complete flow from the beginning: General Setup, Hunt Features, Review Hunt, then Create link &amp; share.</p></div>
        ...
        {!independent && reviewRefreshFailure && <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-rose-50 p-4"><p className="text-sm font-semibold text-rose-700" role="alert">{reviewRefreshFailure}</p><button className="min-h-10 rounded-lg border border-rose-200 bg-white px-4 text-sm font-bold" onClick={() => void openReview()} type="button">Retry</button></div>}
        ...
        <section className="mt-8" aria-labelledby="features-title">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-700">Block 2</p><h2 className="mt-2 text-2xl font-bold" id="features-title">Hunt Features</h2><p className="mt-1 text-sm text-slate-500">Build what participants will experience during the Hunt.</p></div>
            <div className="min-w-48"><div className="flex justify-between text-xs font-semibold"><span>{completedRequired} of {requiredFeatures.length} complete</span><span>{progress}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full bg-emerald-500" style={{ width: `${progress}%` }} /></div></div>
          </div>
          <div className="mt-5 grid gap-5 lg:grid-cols-[280px_1fr]">
          <nav className="space-y-2" aria-label="Hunt Features">{huntFeatures.map((item, index) => {const disabled=featureDisabled(item.id);return <button className={`flex min-h-12 w-full items-center justify-between rounded-xl border px-4 text-left text-sm font-bold ${disabled?'cursor-not-allowed border-slate-200 bg-slate-100 text-slate-400':active === index ? 'border-emerald-500 bg-emerald-50 text-emerald-900' : 'border-slate-200 bg-white hover:border-emerald-300'}`} disabled={disabled} key={item.id} onClick={() => setActive(index)} type="button"><span><span className="mr-2 text-slate-400">{index + 1}.</span>{item.name}</span>{disabled?<span>Locked</span>:completed.has(item.id)&&<span className="text-emerald-600">✓</span>}</button>})}</nav>

          <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-700">Feature {active + 1} of {huntFeatures.length}</p><h2 className="mt-2 text-2xl font-bold">{feature.name}</h2>
            {checkpointFeature && <><div className="mt-5 flex gap-2 overflow-x-auto pb-1" aria-label="Checkpoints">{checkpoints.map((label, index) => <button aria-label={label==='F'?'FinishPoint':`Checkpoint ${label}`} className={`grid h-10 w-10 shrink-0 place-items-center rounded-full border-2 text-xs font-semibold ${checkpoint===index?'border-slate-950 bg-slate-950 text-white':checkpointSelections[feature.id]?.[index]!==undefined||challengeSources[index]==='own'?'border-emerald-500 bg-emerald-50 text-emerald-800':'border-slate-300 bg-white text-slate-600'}`} key={label} onClick={()=>setCheckpoint(index)} type="button">{label}</button>)}</div><div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-xl bg-emerald-50 px-4 py-3"><div><span className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Signal default</span><strong className="ml-2 text-sm">{signalCheckpointNames[checkpoint]}</strong></div><span className="text-xs text-emerald-900">Change only if needed</span></div></>}
            <div className={`mt-5 grid gap-6 xl:grid-cols-[1fr_280px] ${feature.id==='rewards'||feature.id==='map'?'hidden':''}`}>
              <div>{feature.id==='personal'?<><fieldset><legend className="text-sm font-bold">Challenge source</legend><div className="mt-3 flex flex-wrap gap-4">{[['template','Template challenge'],['own','My challenge']].map(([value,label])=><label className="flex items-center gap-2 text-sm font-bold" key={value}><input checked={(challengeSources[checkpoint]??'template')===value} className="h-5 w-5 accent-emerald-500" name={`source-${checkpoint}`} onChange={()=>setChallengeSources({...challengeSources,[checkpoint]:value as 'template'|'own'})} type="radio"/>{label}</label>)}</div></fieldset>{(challengeSources[checkpoint]??'template')==='template'?<label className="mt-4 block text-sm font-bold">Template challenge<select className={controlClass} value={chosenIndex} onChange={event=>setCheckpointSelections({...checkpointSelections,personal:{...(checkpointSelections.personal??{}),[checkpoint]:Number(event.target.value)}})}>{feature.templates.map((item,index)=><option key={`${item.name}-${index}`} value={index}>{item.name}{index===checkpoint?' · Signal default':''}</option>)}</select></label>:<CustomChallengeEditor value={customChallenge} onChange={next=>setCustomChallenges({...customChallenges,[checkpoint]:next})}/>}</>:<label className="text-sm font-bold">{checkpointFeature?'Challenge or navigation template':'Template'}<select className={controlClass} value={chosenIndex} onChange={(event) => checkpointFeature?setCheckpointSelections({...checkpointSelections,[feature.id]:{...(checkpointSelections[feature.id]??{}),[checkpoint]:Number(event.target.value)}}):setSelected({ ...selected, [feature.id]: Number(event.target.value) })}>{feature.templates.map((item, index) => <option key={`${item.name}-${index}`} value={index}>{item.name}{checkpointFeature&&index===defaultIndex?' · Signal default':''}</option>)}</select></label>}<details className="mt-5 rounded-xl bg-slate-50 p-4" open><summary className="cursor-pointer text-sm font-bold">Example</summary><p className="mt-3 text-sm leading-6 text-slate-700">{template.note}</p><p className="mt-2 text-sm text-slate-500">{template.example}</p></details></div>
              <aside><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Preview</p><StudentPreview example={template.example} featureId={feature.id} name={template.name} /></aside>
            </div>
            {feature.id === 'map' && <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_280px]"><div className="rounded-xl bg-slate-50 p-5"><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Fixed Hunt feature</p><h3 className="mt-2 text-lg font-bold">A chronological record of the Hunt</h3><p className="mt-2 text-sm leading-6 text-slate-600">Participants see completed locations, team discoveries, routes unlocked and important mission events. Private personal answers stay hidden.</p><ol className="mt-4 space-y-2 text-sm"><li className="rounded-lg border border-emerald-200 bg-white p-3"><strong>1 · Matthias Rex Statue</strong><span className="ml-2 text-emerald-700">UNLOCKED</span></li><li className="rounded-lg border border-slate-200 bg-white p-3"><strong>2 · Stone Gate</strong><span className="ml-2 text-slate-500">Route recovered</span></li></ol></div><StudentPreview example="Matthias Rex Statue: UNLOCKED → Stone Gate: route recovered." featureId="history" name="Mission History" /></div>}
            ...
            {feature.id === 'positions' && (independent?<div className="mt-6 rounded-xl border border-emerald-200 bg-emerald-50 p-5"><p className="text-xs font-semibold uppercase tracking-wide text-emerald-800">Creator-verified route</p><h3 className="mt-2 text-lg font-bold">Cluj-Napoca centre · 7 fixed checkpoints</h3><p className="mt-2 text-sm text-emerald-950">For safety, checkpoint positions and the 30 m detection radius cannot be changed in an Independent Hunt.</p></div>:<CheckpointMapEditor onVerifiedChange={setVerifiedPositionCount} />)}
            <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-5"><button className="min-h-12 rounded-xl border border-slate-300 px-5 font-bold disabled:opacity-40" disabled={active === 0} onClick={previousFeature} type="button">Previous</button>{allComplete?<button className="min-h-12 rounded-xl bg-emerald-500 px-6 font-bold disabled:bg-slate-300" disabled={!generalComplete} onClick={() => void openReview()} type="button">{generalComplete?'Review Hunt':'Complete General Setup first'}</button>:<button className="min-h-12 rounded-xl bg-emerald-500 px-6 font-bold disabled:bg-slate-300" disabled={feature.id === 'positions' && !independent && !positionsVerified} onClick={saveAndContinue} type="button">{feature.id === 'positions' && !independent && !positionsVerified ? `Verify all checkpoints (${verifiedPositionCount}/7)` : 'Save & continue'}</button>}</div>
          </article>
        </div></section>
        ...
        <section className={`mt-8 rounded-2xl border p-5 shadow-sm sm:p-6 ${(!independent && persistedHunt)||(generalComplete&&allComplete)?'border-emerald-300 bg-white':'border-slate-200 bg-slate-50'}`} aria-labelledby="review-hunt-title">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div><p className={`text-xs font-semibold uppercase tracking-[0.16em] ${(!independent && persistedHunt)||(generalComplete&&allComplete)?'text-emerald-700':'text-slate-400'}`}>Block 3</p><h2 className={`mt-2 text-2xl font-bold ${(!independent && persistedHunt)||(generalComplete&&allComplete)?'text-slate-950':'text-slate-500'}`} id="review-hunt-title">Review Hunt</h2><p className="mt-1 text-sm text-slate-500">Check the complete setup before creating participant access.</p></div>
            <button className="min-h-12 rounded-xl bg-emerald-500 px-6 font-bold disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-500" disabled={reviewRefreshing || (independent ? !generalComplete||!allComplete : !persistedHunt)} onClick={() => void openReview()} type="button">{reviewRefreshing ? 'Refreshing saved Hunt…' : independent ? (generalComplete&&allComplete?'Review Hunt':`Locked · ${completedRequired}/${requiredFeatures.length} features`) : 'Review saved Hunt'}</button>
          </div>
        </section>
        ...
        <section className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm sm:p-6" aria-labelledby="share-hunt-title">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Block 4</p><h2 className="mt-2 text-2xl font-bold text-slate-500" id="share-hunt-title">Create link &amp; share</h2><p className="mt-1 text-sm text-slate-500">Participant link, Hunt code and QR become available after Review Hunt.</p></div>
            <button className="min-h-12 cursor-not-allowed rounded-xl bg-slate-200 px-6 font-bold text-slate-500" disabled type="button">Locked · Review first</button>
          </div>
        </section>
      </div>
    </main>
