    ...
    <main className="min-h-dvh bg-[#07110f] px-5 py-5 text-white">
      <div className="mx-auto flex min-h-[calc(100dvh-2.5rem)] w-full max-w-lg flex-col">
        ...
        <section className="flex flex-1 flex-col justify-center py-6">
          ...
          <h1 className="mt-2 text-4xl font-black tracking-tight">Team Aurora is ready</h1>

          <div className="mt-5 grid grid-cols-2 gap-3 rounded-2xl border border-emerald-300/20 bg-emerald-300/[0.06] p-4">
            <div><span className="text-xs font-bold uppercase tracking-wide text-emerald-300">Your role</span><p className="mt-1 font-black">Signal Decoder</p></div>
            <div><span className="text-xs font-bold uppercase tracking-wide text-emerald-300">Starting score</span><p className="mt-1 text-2xl font-black">500</p></div>
          </div>
          ...
          <fieldset className="mt-4">
            ...
            <div className="mt-2 grid grid-cols-3 gap-2">
              {difficulties.map((option) => (
                <button aria-pressed={difficulty === option.id} className={`min-h-14 rounded-xl border px-2 font-black ${difficulty === option.id ? 'border-emerald-300 bg-emerald-300/15 text-emerald-200' : 'border-white/15 bg-white/[0.03] text-slate-300'}`} key={option.id} onClick={() => setDifficulty(option.id)} type="button">
          ...
          <button className="mt-4 min-h-16 rounded-xl bg-emerald-400 px-6 font-black text-slate-950 disabled:cursor-not-allowed disabled:opacity-35" disabled={!consent} onClick={() => navigate('/play/template-1', { state: { startNewMission: true, difficulty } })}>Begin the Hunt</button>
