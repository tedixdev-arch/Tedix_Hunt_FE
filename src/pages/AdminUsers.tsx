  ...
  return <AdminShell active="users">
    <div className="mt-6 grid gap-5 lg:grid-cols-[1fr_380px]">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="text-2xl font-bold">Admin identities</h2>
        ...
        {!loading && !listError && <div className="mt-5 divide-y divide-slate-100">{admins.map(admin => <article className="grid gap-1 py-4 sm:grid-cols-[1fr_1fr_auto] sm:items-center sm:gap-4" key={admin.id}><strong>{admin.name || 'Unnamed Admin'}</strong><span className="text-sm text-slate-600">{admin.email}</span><span className="w-fit rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold">{activationLabels[admin.activationState]}</span></article>)}</div>}
      </section>
      <section className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <form className="mt-5" onSubmit={submit}>
          ...
          <button className="mt-5 min-h-12 w-full rounded-xl bg-emerald-500 px-5 font-bold disabled:cursor-wait disabled:opacity-60" disabled={isSubmitting} type="submit">{isSubmitting ? 'Adding Admin…' : 'Add Admin'}</button>
        </form>
        {result && <div className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50 p-4" role="status">
          {!result.activationRequired && <p className="font-bold text-emerald-900">Admin access granted.</p>}
          {result.activationRequired && <>
            <p className="font-bold text-emerald-900">Admin invitation created.</p>
