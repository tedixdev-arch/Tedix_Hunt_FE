...
export function AdminUsersPage() {
  ...
  async function submitEdit(event: FormEvent) {
    ...
    if (!nextName || !nextEmail) { setManagementError('Name and email are required.'); return }
    await manage(() => adminUsersApi.updateUser(managed!.id, { name: nextName, email: nextEmail }), 'User details updated.')
  }
    ...
    {managed && <div aria-labelledby="manage-user-title" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-6" role="dialog">
      <section className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
        ...
        {mode === 'overview' && <div className="mt-6 space-y-6">
          ...
          <div><h3 className="font-bold">Global capabilities</h3><div className="mt-3 grid gap-2 sm:grid-cols-3">{roles.map(role => { const held = managed.roles.includes(role); const ownAdmin = managed.id === currentAdmin?.id && role === 'admin'; return <button className={`rounded-lg border px-3 py-2 text-sm font-bold ${held ? 'border-emerald-500 bg-emerald-50' : 'border-slate-300'}`} disabled={working || ownAdmin} key={role} onClick={() => void manage(() => held ? adminUsersApi.removeRole(managed.id, role) : adminUsersApi.grantRole(managed.id, role), `${roleContent[role].label} capability ${held ? 'removed' : 'granted'}.`)} type="button">{held ? `Remove ${roleContent[role].label}` : `Grant ${roleContent[role].label}`}</button> })}</div></div>
