...
export function JoinHuntPage() {
  ...
  return <ParticipantShell><section className="flex flex-1 flex-col justify-center py-8"><p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-300">Join your team</p><h1 className="mt-3 text-4xl font-black tracking-tight">Enter Signal: Cluj Napoca</h1><div className="mt-5 rounded-xl border border-emerald-300/20 bg-emerald-300/[0.06] p-4"><p className="text-xs font-bold uppercase tracking-wide text-emerald-300">Hunt found</p><p className="mt-1 font-black">Signal: Cluj Napoca</p><p className="mt-2 text-sm text-slate-400">Start: Matthias Rex Statue, Cluj-Napoca</p></div>
    <div className="mt-5 grid grid-cols-3 gap-1 rounded-xl bg-white/[0.06] p-1" aria-label="Participant access type">{(['guest','login','register'] as Mode[]).map(item => <button key={item} className={`min-h-11 rounded-lg text-xs font-black capitalize ${mode === item ? 'bg-emerald-400 text-slate-950' : 'text-slate-300'}`} onClick={() => { setMode(item); setError('') }} type="button">{item === 'login' ? 'Sign in' : item}</button>)}</div>
    <form className="mt-4 space-y-4" onSubmit={join}><label className="block text-sm font-bold" htmlFor="hunt-code">Hunt code<input autoCapitalize="characters" autoComplete="one-time-code" className="mt-2 min-h-14 w-full rounded-xl border border-white/15 bg-white/[0.06] px-4 text-center text-lg font-black uppercase tracking-[0.18em]" id="hunt-code" onChange={event => setCode(event.target.value.toUpperCase())} value={code}/></label>
      ...
      {error && <p className="rounded-lg bg-amber-300/10 px-3 py-2 text-sm font-bold text-amber-200" role="alert">{error}</p>}<button className="min-h-16 w-full rounded-xl bg-emerald-400 px-6 font-black text-slate-950 disabled:opacity-60" disabled={isSubmitting || isBootstrapping} type="submit">{isSubmitting ? 'Please wait…' : mode === 'guest' ? 'Continue as guest' : mode === 'login' ? 'Sign in & join' : 'Create account & join'}</button></form><Link className="mt-3 min-h-11 py-3 text-center text-sm font-semibold text-slate-400" to="/">Back to home</Link></section></ParticipantShell>
}
