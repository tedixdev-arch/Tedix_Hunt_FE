...
function StatusBadge({ application }: { application: OrganizerApplication }) {
  ...
  return <span className={`w-fit rounded-full px-3 py-1 text-xs font-semibold ${color}`}>{statusLabels[application.status]}</span>
}
...
export function AdminOrganizerApplicationsPage() {
  ...
  return <AdminShell active="organizer-applications">
    <div className="mt-6">
      <h2 className="text-3xl font-bold">Organizer Applications</h2>
      ...
      <div className="mt-5 flex flex-wrap gap-2" aria-label="Application status filters">
        {filters.map(item => <button aria-pressed={filter === item.value} className={`min-h-10 rounded-lg px-4 text-sm font-bold ${filter === item.value ? 'bg-slate-950 text-white' : 'border border-slate-300 bg-white text-slate-700'}`} key={item.value} onClick={() => changeFilter(item.value)} type="button">{item.label}</button>)}
      </div>
    </div>
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_400px]">
      ...
      <aside className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" aria-label="Application details">
        ...
        {selected && <>
          <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Applicant</p><h3 className="mt-1 text-2xl font-bold">{selected.name}</h3></div><StatusBadge application={selected} /></div>
          <dl className="mt-5 space-y-4 text-sm">
            <Detail label="Email" value={selected.email} />
            {selected.phone && <Detail label="Phone" value={selected.phone} />}
            <div className="border-t border-slate-100 pt-4"><dt className="text-xs font-semibold uppercase tracking-wide text-slate-400">Organization</dt><dd className="mt-1 font-bold">{selected.organizationName}</dd><dd className="mt-1 text-slate-600">{organizationTypeLabels[selected.organizationType]}</dd></div>
...
function Detail({ label, value }: { label: string; value: string }) {
  return <div><dt className="text-xs font-semibold uppercase tracking-wide text-slate-400">{label}</dt><dd className="mt-1 whitespace-pre-wrap font-semibold text-slate-800">{value}</dd></div>
}
