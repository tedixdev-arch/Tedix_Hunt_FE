...
export function AdminActivationPage() {
  ...
  return <main className="grid min-h-dvh place-items-center bg-slate-950 px-5 py-10 text-slate-950"><section className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl sm:p-8"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Admin activation</p><h1 className="mt-2 text-3xl font-bold">Create your password</h1><p className="mt-2 text-sm text-slate-600">You received this link because an existing Admin granted you Admin access.<br />Create your password to activate your account.</p><p className="mt-2 text-sm text-slate-600">Use at least 8 characters to activate your Admin account.</p>
    {!token ? <><p className="mt-6 rounded-lg bg-amber-50 p-3 font-semibold text-amber-800" role="alert">Invalid activation link.</p><Link className="mt-5 inline-flex font-bold underline" to="/admin/sign-in">Go to Admin sign in</Link></> : <form className="mt-6" onSubmit={submit}>
      ...
      <button className="mt-5 min-h-12 w-full rounded-xl bg-emerald-500 px-5 font-bold disabled:cursor-wait disabled:opacity-60" disabled={isSubmitting} type="submit">{isSubmitting ? 'Activating…' : 'Activate Admin account'}</button>
    </form>}
  </section></main>
}
