...
function PersonalChallengeOverrides({drafts,onChange}:{drafts:ChallengeDraft[];onChange:(drafts:ChallengeDraft[])=>void}){
  ...
  return <div className="sm:col-span-2"><div className="flex gap-2 overflow-x-auto pb-2">{drafts.map((item,index)=><button className={`shrink-0 rounded-full px-3 py-2 text-xs font-semibold ${checkpoint===index?'bg-slate-950 text-white':item.enabled?'bg-emerald-100 text-emerald-800':'bg-slate-100 text-slate-600'}`} key={index} onClick={()=>setCheckpoint(index)} type="button">Checkpoint {index+1}{item.enabled?' ✓':''}</button>)}</div><div className="mt-3 rounded-xl border border-slate-200 p-4"><div className="flex items-center justify-between gap-4"><div><p className="font-bold">Checkpoint {checkpoint+1} personal challenge</p><p className="mt-1 text-sm text-slate-500">The verified route and team challenge do not change.</p></div><label className="flex shrink-0 items-center gap-2 text-sm font-bold"><input checked={draft.enabled} className="h-5 w-5 accent-emerald-500" onChange={e=>update({enabled:e.target.checked})} type="checkbox"/>Use my own</label></div>{draft.enabled&&<div className="mt-4 grid gap-4"><label className={labelClass}>Challenge format<select className={inputClass} value={draft.type} onChange={e=>update({type:e.target.value as ChallengeDraft['type'],answers:['','','']})}>{challengeTypes.map(type=><option key={type}>{type}</option>)}</select></label><label className={labelClass}>Question<input className={inputClass} maxLength={180} onChange={e=>update({question:e.target.value})} placeholder="Write a short, clear question" value={draft.question}/></label>{labels.map((label,index)=><label className={labelClass} key={label}>{label}<input className={inputClass} maxLength={80} onChange={e=>update({answers:draft.answers.map((answer,itemIndex)=>itemIndex===index?e.target.value:answer)})} placeholder={label} value={draft.answers[index]??''}/></label>)}<div className="rounded-xl bg-emerald-950 p-4 text-white"><p className="text-[10px] font-bold uppercase tracking-wide text-emerald-300">Participant preview</p><h4 className="mt-2 font-bold">{draft.question||'Your question appears here'}</h4><div className="mt-3 grid gap-2">{labels.map((_,index)=><span className="rounded-lg border border-white/15 px-3 py-2 text-sm" key={index}>{draft.answers[index]||`Option ${index+1}`}</span>)}</div></div><p className="text-xs leading-5 text-slate-500">Structured text only. No links, uploads or requests for personal information. Participants can report a challenge.</p></div>}</div></div>
}
  ...
  return (
    <SetupShell eyebrow="Recommended · Quick Setup" title="Launch a proven hunt" description="Choose a complete Competition Template and add only the details needed to run it locally.">
      <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" aria-labelledby="template-title">
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-700">Block 1</p><h2 className="mt-2 text-2xl font-bold" id="template-title">Select Hunt Template</h2><p className="mt-1 text-sm text-slate-500">Choose a complete, tested Hunt. Its experience settings stay locked.</p>
        <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_300px]"><div><label className={labelClass}>Competition template<select className={inputClass} value={template} onChange={event => { setTemplate(event.target.value); setSettings({...settings,name:event.target.value}) }}><option>Signal: Cluj Napoca</option><option>The City Code</option></select></label><p className="mt-3 text-sm leading-6 text-slate-500">The story, route, checkpoints, team challenges, scoring and results are locked to this approved template.</p><div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-wide text-emerald-800">Creator-verified route</p><p className="mt-1 font-bold">Cluj-Napoca centre · 7 fixed checkpoints</p></div><span className="rounded-full bg-emerald-500 px-3 py-1 text-xs font-semibold text-white">Approved</span></div><div className="relative mt-4 h-40 overflow-hidden rounded-xl border border-emerald-200 bg-[#e7eee8]"><div className="absolute inset-0 opacity-60" style={{backgroundImage:'linear-gradient(25deg, transparent 47%, #b7c7bc 48%, #b7c7bc 51%, transparent 52%)',backgroundSize:'95px 65px'}}/>{[12,25,38,51,64,77,89].map((left,index)=><span className="absolute grid h-7 w-7 -translate-x-1/2 place-items-center rounded-full border-2 border-white bg-emerald-500 text-[10px] font-bold text-white shadow" key={left} style={{left:`${left}%`,top:`${index%2?55:28}%`}}>{index+1}</span>)}</div><dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-slate-500">Route</dt><dd className="font-bold">2.4 km · 35 min walk</dd></div><div><dt className="text-slate-500">Detection radius</dt><dd className="font-bold">30 metres</dd></div><div><dt className="text-slate-500">Safety review</dt><dd className="font-bold">28 Aug 2026</dd></div><div><dt className="text-slate-500">Creator</dt><dd className="font-bold">Tedix Design Team</dd></div></dl>{!independent&&<Link className="mt-4 inline-flex text-sm font-bold text-emerald-800 underline" to="/organizer/hunts/new/guided">Need another route? Use Custom Setup</Link>}</div></div><aside className="rounded-xl bg-[#061812] p-4 text-white"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-emerald-300">Template preview</p><h3 className="mt-3 text-xl font-bold">{template}</h3><dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-slate-400">Difficulty</dt><dd className="font-bold">User set</dd></div><div><dt className="text-slate-400">Checkpoints</dt><dd className="font-bold">6 + Final</dd></div><div><dt className="text-slate-400">Format</dt><dd className="font-bold">Single or Team</dd></div><div><dt className="text-slate-400">Duration</dt><dd className="font-bold">90 min</dd></div></dl><p className="mt-4 border-t border-white/10 pt-3 text-xs leading-5 text-slate-300">Personal challenges · Team challenges when enabled · Progressive results</p></aside></div>
        <div className="mt-5 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{[['Story','Restore the Signal'],['Checkpoint order','Creator fixed'],['Scoring','Balanced · fixed'],['Results','Progressive leaderboard'],['Navigation','Verified map'],['Team challenges','Included for Team Hunters'],['Final mission','Restore the transmitter'],['Rewards',independent?'Registered only':'Recognition + partner rewards']].map(([label,value])=><div className="rounded-xl border border-slate-200 bg-slate-100 p-3" key={label}><span className="text-[10px] font-bold uppercase tracking-wide text-slate-400">🔒 {label}</span><p className="mt-1 text-sm font-bold text-slate-600">{value}</p></div>)}</div>
      </section>

      <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" aria-labelledby="quick-general-title">
        <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-700">Block 2</p><h2 className="mt-2 text-2xl font-bold" id="quick-general-title">General Setup</h2><p className="mt-1 text-sm text-slate-500">Add only the local details needed to launch this Hunt.</p></div><div className="min-w-48"><div className="flex justify-between text-xs font-semibold"><span>{completed.size} of {steps.length} complete</span><span>{progress}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full bg-emerald-500" style={{width:`${progress}%`}} /></div></div></div>
        <div className="mt-6 grid min-w-0 gap-5 lg:grid-cols-[240px_minmax(0,1fr)]"><nav className="min-w-0 space-y-2" aria-label="Quick Setup sections">{steps.map((step,index)=><button className={`flex min-h-12 w-full items-center justify-between rounded-xl border px-4 text-left text-sm font-bold ${active===index?'border-emerald-500 bg-emerald-50 text-emerald-900':'border-slate-200 bg-slate-50'}`} key={step} onClick={()=>setActive(index)} type="button"><span>{index+1}. {step}</span>{completed.has(index)&&<span className="text-emerald-600">✓</span>}</button>)}</nav>
          <article className="min-w-0 rounded-xl border border-slate-200 p-5"><p className="text-xs font-semibold uppercase tracking-wide text-emerald-700">General {active+1} of {steps.length}</p><h3 className="mt-2 text-xl font-bold">{steps[active]}</h3><div className="mt-5 grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_240px]"><div className="grid min-w-0 gap-4 sm:grid-cols-2">
            {active===0&&<><label className={`${labelClass} sm:col-span-2`}>Hunt name<input className={inputClass} value={settings.name} onChange={e=>setSettings({...settings,name:e.target.value})}/></label><label className={labelClass}>Country<select className={inputClass} value={settings.country} onChange={e=>setSettings({...settings,country:e.target.value})}><option>Romania</option><option>United Kingdom</option></select></label><label className={labelClass}>County / region<select className={inputClass} value={settings.county} onChange={e=>setSettings({...settings,county:e.target.value})}><option>Cluj</option><option>Bucharest</option></select></label><label className={labelClass}>City<select className={inputClass} value={settings.location} onChange={e=>setSettings({...settings,location:e.target.value})}><option>Cluj Napoca</option></select></label><label className={labelClass}>Language<select className={inputClass} value={settings.language} onChange={e=>setSettings({...settings,language:e.target.value})}><option>English</option><option>Romanian</option></select></label><label className={labelClass}>Hunt date<input className={inputClass} value={settings.date} onChange={e=>setSettings({...settings,date:e.target.value})} type="date"/></label><label className={labelClass}>Start time<input className={inputClass} value={settings.time} onChange={e=>setSettings({...settings,time:e.target.value})} type="time"/></label><div className="rounded-xl bg-slate-50 p-4 text-sm"><p className="font-bold text-slate-500">Fixed duration</p><p className="mt-2 font-bold">{settings.duration} minutes</p></div><label className={labelClass}>Local contact<input className={inputClass} value={settings.contact} onChange={e=>setSettings({...settings,contact:e.target.value})}/></label></>}
            ...
            {active===3&&<div className="sm:col-span-2"><div className="rounded-xl bg-slate-50 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Access</p><p className="mt-2 font-bold">{settings.access}</p><p className="mt-1 text-sm text-slate-600">Create one link for participants.</p></div>{!linkCreated?<button className="mt-4 min-h-12 w-full rounded-xl bg-emerald-500 px-5 font-bold" onClick={()=>setLinkCreated(true)} type="button">Create Hunt link</button>:<div className="mt-4 space-y-2"><input aria-label="Hunt invitation link" className="min-h-12 w-full rounded-xl border border-emerald-300 bg-white px-4 text-sm" readOnly value={invitationLink}/><div className="flex gap-2"><button className="min-h-12 flex-1 rounded-xl border border-emerald-400 bg-white px-4 font-bold" onClick={copyLink} type="button">Copy link</button><button className="min-h-12 flex-1 rounded-xl bg-slate-950 px-4 font-bold text-white" onClick={shareHunt} type="button">Share</button></div>{linkMessage&&<p className="text-sm font-bold text-emerald-700" role="status">✓ {linkMessage}</p>}</div>}</div>}
          </div><aside className="rounded-xl bg-[#061812] p-4 text-white"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-emerald-300">Setup preview</p><h4 className="mt-3 font-bold">{steps[active]}</h4><p className="mt-3 whitespace-pre-line text-sm leading-6 text-slate-200">{active===0?`${settings.country} · ${settings.county}\n${settings.location} · ${settings.language}\n${settings.date} · ${settings.time}`:active===1?`${settings.participants} participants\n${settings.format}${settings.format==='Team Hunters'?` · Teams of ${settings.teamSize}`:''}\n${settings.access}`:active===2?`${challenges.filter(item=>item.enabled).length} of 7 custom personal challenges\nVerified route unchanged`:`${settings.name}\n${settings.date} · ${settings.time}\nJoin Hunt`}</p></aside></div><div className="mt-6 flex justify-between border-t border-slate-100 pt-5"><button className="min-h-12 rounded-xl border border-slate-300 px-5 font-bold disabled:opacity-40" disabled={active===0} onClick={()=>setActive(active-1)} type="button">Previous</button><button className="min-h-12 rounded-xl bg-emerald-500 px-6 font-bold disabled:cursor-not-allowed disabled:bg-slate-300" disabled={active===3&&!linkCreated} onClick={saveStep} type="button">{active===3?'Create Hunt':'Save & continue'}</button></div></article>
        </div>
      </section>
    </SetupShell>
  )
