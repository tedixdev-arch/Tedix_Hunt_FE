  ...
  return (
    <header className="border-b border-slate-200 bg-white">
      <div className="mx-auto flex min-h-16 max-w-6xl items-center justify-between px-5 sm:px-8">
        ...
        {showProfile ? <div className="flex min-w-0 items-center gap-1 sm:gap-3">{user && currentWorkspace && <Link aria-label={`Switch workspace: ${currentWorkspace}`} className="min-h-10 shrink-0 rounded-lg px-2 py-2.5 text-xs font-bold text-emerald-800 hover:bg-emerald-50 sm:px-3 sm:text-sm" state={{ from: location.pathname }} to="/workspaces"><span className="hidden sm:inline">Switch Workspace: </span>{currentWorkspace} <span aria-hidden="true">→</span></Link>}<span className="hidden truncate text-sm text-slate-500 sm:block">{user?.name ?? 'Cluj Youth Centre'}</span>{user ? <button className="min-h-10 shrink-0 rounded-lg px-2 text-xs font-bold text-slate-600 hover:bg-slate-100 sm:px-3 sm:text-sm" onClick={() => void signOut()} type="button">Log out</button> : <span className="grid h-9 w-9 place-items-center rounded-full bg-emerald-100 text-xs font-semibold text-emerald-800">OC</span>}</div> : <Link className="text-sm font-bold text-slate-500 hover:text-slate-900" to="/">Back to home</Link>}
      </div>
    </header>
  )
}
...
export function OrganizerSignInPage() {
  return <main className="h-dvh overflow-y-auto bg-slate-100 text-slate-950"><OrganizerHeader/><div className="mx-auto max-w-4xl px-5 py-10 sm:px-8"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Organizer workspace</p><h1 className="mt-3 text-4xl font-bold tracking-tight">How will you organize?</h1><p className="mt-3 max-w-2xl text-slate-600">Choose the access that matches your Hunt. Both use the same clear Set the Hunt flow.</p><div className="mt-8 grid gap-4 md:grid-cols-2"><article className="rounded-2xl border border-emerald-300 bg-white p-6 shadow-sm"><span className="text-xs font-semibold uppercase tracking-wide text-emerald-700">Registered Organizer</span><h2 className="mt-3 text-2xl font-bold">For schools and organizations</h2><p className="mt-3 leading-6 text-slate-600">Approved teachers, schools, NGOs and community partners can set every Hunt feature and configure rewards.</p><Link className="mt-6 flex min-h-14 items-center justify-center rounded-xl bg-emerald-500 px-5 font-bold" to="/organizer/registered">Sign in</Link><Link className="mt-2 flex min-h-11 w-full items-center justify-center text-sm font-bold text-emerald-800" to="/organizer/apply">Apply as a Registered Organizer</Link></article><article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><span className="text-xs font-semibold uppercase tracking-wide text-slate-500">Independent Organizer</span><h2 className="mt-3 text-2xl font-bold">A private Hunt with friends</h2><p className="mt-3 leading-6 text-slate-600">Confirm your email and use the full Hunt builder with a verified route. Rewards remain unavailable.</p><Link className="mt-6 flex min-h-14 items-center justify-center rounded-xl border border-slate-900 px-5 font-bold" to="/organizer/independent">Create an Independent Hunt</Link></article></div></div></main>
}
  ...
  return (
    <main className="h-dvh overflow-y-auto bg-slate-100 text-slate-950">
      ...
      <div className="mx-auto flex min-h-[calc(100dvh-4rem)] max-w-md flex-col justify-center px-5 py-10">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Organizer workspace</p>
        <h1 className="mt-3 text-4xl font-bold tracking-tight">Sign in to manage your hunts</h1>
        ...
        <form className="mt-7 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" onSubmit={signIn}>
          ...
          <button className="mt-5 min-h-14 w-full rounded-xl bg-emerald-500 px-5 font-bold text-slate-950 hover:bg-emerald-400 disabled:cursor-wait disabled:opacity-60" disabled={isSubmitting} type="submit">{isSubmitting ? 'Signing in…' : 'Sign in'}</button>
        </form>
      </div>
    </main>
  )
...
export function IndependentOrganizerPage() {
  ...
  return <main className="h-dvh overflow-y-auto bg-slate-100 text-slate-950"><OrganizerHeader/><div className="mx-auto max-w-md px-5 py-10"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Independent Hunt</p><h1 className="mt-3 text-4xl font-bold tracking-tight">Confirm your email</h1><p className="mt-3 leading-6 text-slate-600">Create a private Hunt for friends using a Creator-verified route.</p><section className="mt-7 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><label className="text-sm font-bold">Email address<input className="mt-2 min-h-14 w-full rounded-xl border border-slate-300 px-4" onChange={e=>{setEmail(e.target.value);setConfirmed(false)}} type="email" value={email}/></label><button className="mt-4 min-h-12 w-full rounded-xl border border-emerald-500 font-bold text-emerald-800" disabled={!email.includes('@')} onClick={()=>setConfirmed(true)} type="button">{confirmed?'✓ Email confirmed':'Send confirmation code'}</button><div className="mt-5 rounded-xl bg-amber-50 p-4 text-sm leading-6 text-amber-950"><strong>Independent activity</strong><p className="mt-1">Independent Hunts are private, unofficial activities. The Organizer and participants are responsible for following local rules, staying safe and using the verified route as intended.</p></div><label className="mt-4 flex items-start gap-3 text-sm leading-6"><input checked={accepted} className="mt-1 h-5 w-5 accent-emerald-500" onChange={e=>setAccepted(e.target.checked)} type="checkbox"/><span>I understand that this is an independent activity and I am responsible for organizing it safely.</span></label><button className="mt-5 min-h-14 w-full rounded-xl bg-emerald-500 font-bold disabled:bg-slate-300" disabled={!confirmed||!accepted} onClick={()=>navigate('/organizer/hunts/new?mode=independent')} type="button">Set the Hunt</button></section></div></main>
}
    ...
    <main className="h-dvh overflow-y-auto bg-slate-100 text-slate-950">
      ...
      <div className="mx-auto max-w-6xl px-5 py-7 sm:px-8 sm:py-10">
        <div className="gap-6 sm:flex sm:items-end sm:justify-between">
          <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Organizer workspace</p><h1 className="mt-2 text-4xl font-bold tracking-tight">My Hunts</h1><p className="mt-3 text-slate-600">See what needs attention and take the next action.</p></div>
          <Link className="mt-6 flex min-h-14 items-center justify-center rounded-xl bg-emerald-500 px-6 font-bold text-slate-950 shadow-sm hover:bg-emerald-400 sm:mt-0" to="/organizer/hunts/new">+ Set up a hunt</Link>
        </div>
        ...
        <section className="mt-8" aria-labelledby="hunt-list-title">
          <div className="flex items-center justify-between"><h2 className="text-xl font-bold" id="hunt-list-title">All hunts</h2><span className="text-sm text-slate-500">{isLoading ? 'Loading…' : `${hunts.length} ${hunts.length === 1 ? 'hunt' : 'hunts'}`}</span></div>
          <div className="mt-4 grid gap-3 lg:grid-cols-2">
            ...
            {!isLoading && !loadError && hunts.length === 0 && <div className="rounded-xl border border-slate-200 bg-white p-6"><p className="font-bold text-slate-800">No Hunts yet.</p><p className="mt-1 text-sm text-slate-500">Set up a Hunt when you're ready to get started.</p></div>}
            {!isLoading && !loadError && hunts.map((hunt) => (
              <article className={`rounded-xl border bg-white p-4 shadow-sm ${hunt.status === 'active' ? 'border-emerald-400' : hunt.status === 'paused' ? 'border-amber-300' : 'border-slate-200'}`} key={hunt.id}>
                <div className="flex items-start justify-between gap-3">
                  <div><h3 className="font-bold">{hunt.name}</h3><p className="mt-1 text-sm text-slate-500">{formatUpdatedAt(hunt.updatedAt)}</p></div>
                  <span className={`shrink-0 rounded-full px-3 py-1 text-xs font-semibold ${statusStyles[hunt.status]}`}>{statusLabels[hunt.status]}</span>
                </div>
