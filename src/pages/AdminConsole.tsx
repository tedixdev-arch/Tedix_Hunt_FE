...
export function AdminTemplateReviewPage() {
  ...
  if(decision)return <AdminShell active="templates"><section className={`mt-6 rounded-2xl border p-7 ${decision==='approved'?'border-emerald-300 bg-emerald-50':'border-amber-300 bg-amber-50'}`} role="status"><div className={`grid h-12 w-12 place-items-center rounded-full text-2xl font-bold text-white ${decision==='approved'?'bg-emerald-500':'bg-amber-500'}`}>✓</div><p className="mt-5 text-xs font-semibold uppercase tracking-wide">Decision recorded</p><h2 className="mt-2 text-3xl font-bold">{decision==='approved'?'Template approved':'Changes requested'}</h2><p className="mt-3">{decision==='approved'?'Signal: Cluj Napoca is now available to Organizers in Quick Setup.':'The template is unlocked for the Creator with your review notes.'}</p><Link className="mt-6 inline-flex min-h-12 items-center rounded-xl bg-slate-950 px-5 font-bold text-white" to="/admin">Return to Dashboard</Link></section></AdminShell>
  return <AdminShell active="templates"><div className="mt-6 flex flex-wrap items-start justify-between gap-4"><div><Link className="text-sm font-bold text-slate-500" to="/admin">← Dashboard</Link><p className="mt-5 text-xs font-semibold uppercase tracking-wide text-amber-700">Awaiting review</p><h2 className="mt-2 text-3xl font-bold">Signal: Cluj Napoca</h2><p className="mt-2 text-slate-600">Tedix Design Team · Mathematics · Ages 12–16 · Version 1.0</p></div><div className="flex gap-2"><button className="min-h-11 rounded-xl border border-slate-300 bg-white px-4 text-sm font-bold" type="button">Edit template</button><button className="min-h-11 rounded-xl border border-red-300 bg-red-50 px-4 text-sm font-bold text-red-800" type="button">Lock</button></div></div><section className="mt-6 grid gap-5 lg:grid-cols-[1fr_340px]"><div className="space-y-4"><article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="text-xl font-bold">Template summary</h3><dl className="mt-4 grid gap-4 text-sm sm:grid-cols-3"><div><dt className="text-slate-500">Format</dt><dd className="mt-1 font-bold">Team Hunt</dd></div><div><dt className="text-slate-500">Duration</dt><dd className="mt-1 font-bold">90 minutes</dd></div><div><dt className="text-slate-500">Checkpoints</dt><dd className="mt-1 font-bold">6 + Final</dd></div></dl></article><article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><h3 className="text-xl font-bold">Fixed route and safety</h3><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-800">Creator verified</span></div><div className="relative mt-4 h-64 overflow-hidden rounded-xl border border-slate-300 bg-[#e7eee8]"><div className="absolute inset-0 opacity-65" style={{backgroundImage:'linear-gradient(25deg, transparent 47%, #b7c7bc 48%, #b7c7bc 51%, transparent 52%)',backgroundSize:'100px 70px'}}/>{[10,23,36,49,62,75,88].map((left,index)=><span className="absolute grid h-8 w-8 place-items-center rounded-full border-2 border-white bg-emerald-500 text-xs font-semibold text-white shadow" key={left} style={{left:`${left}%`,top:`${index%2?58:27}%`}}>{index+1}</span>)}</div><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-3"><div><dt className="text-slate-500">Route</dt><dd className="font-bold">2.4 km</dd></div><div><dt className="text-slate-500">Radius</dt><dd className="font-bold">30 m</dd></div><div><dt className="text-slate-500">Safety checks</dt><dd className="font-bold">6 of 6</dd></div></dl></article><article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="text-xl font-bold">Content and participant flow</h3><div className="mt-4 grid gap-3 sm:grid-cols-2">{['Personal challenges · 6','Team challenges · 6','Hints and solutions · Complete','Results and rewards · Complete'].map(item=><div className="rounded-xl bg-slate-50 p-4 text-sm font-bold" key={item}>{item}</div>)}</div><button className="mt-4 min-h-12 w-full rounded-xl border border-slate-300 bg-white font-bold" type="button">Preview complete participant journey</button></article></div><aside className="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h3 className="text-xl font-bold">Review checklist</h3><p className="mt-2 text-sm text-slate-500">Check every area before making a decision.</p><div className="mt-4 space-y-2">{reviewItems.map(item=><label className="flex min-h-12 items-center gap-3 rounded-xl border border-slate-200 p-3 text-sm font-bold" key={item}><input checked={checked.has(item)} className="h-5 w-5 accent-emerald-500" onChange={()=>{const next=new Set(checked);next.has(item)?next.delete(item):next.add(item);setChecked(next)}} type="checkbox"/>{item}</label>)}</div><label className="mt-4 block text-sm font-bold">Review notes<textarea className="mt-2 min-h-24 w-full rounded-xl border border-slate-300 p-3 outline-none focus:border-emerald-500" onChange={e=>setNotes(e.target.value)} placeholder="Required when requesting changes" value={notes}/></label><button className="mt-4 min-h-12 w-full rounded-xl bg-emerald-500 font-bold disabled:cursor-not-allowed disabled:bg-slate-300" disabled={checked.size!==reviewItems.length} onClick={()=>setDecision('approved')} type="button">Approve template</button><button className="mt-2 min-h-12 w-full rounded-xl border border-amber-400 bg-amber-50 font-bold text-amber-900 disabled:cursor-not-allowed disabled:opacity-40" disabled={!notes.trim()} onClick={()=>setDecision('changes')} type="button">Request changes</button><button className="mt-2 min-h-11 w-full text-sm font-bold text-red-700" type="button">Reject template</button></aside></section></AdminShell>
}
...
export function AdminSectionPage({section}:{section:keyof typeof content}) { const data=content[section]; return <AdminShell active={section}><section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-2xl font-bold">{data.title}</h2><p className="mt-2 text-slate-600">{data.description}</p><div className="mt-5 divide-y divide-slate-100">{data.items.map((item,index)=><div className="flex min-h-16 items-center justify-between gap-4 py-3" key={item}><span className="font-bold">{item}</span>{section==='templates'&&index===0?<Link className="rounded-lg bg-slate-950 px-4 py-2 text-sm font-bold text-white" to="/admin/reviews/template-1">Review</Link>:<button className="text-sm font-bold text-slate-500" type="button">View</button>}</div>)}</div></section></AdminShell> }
