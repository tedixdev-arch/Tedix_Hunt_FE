  ...
  return (
    <main className="relative h-dvh overflow-y-auto bg-[#08090a] text-white">
      ...
      <div className="relative z-10 mx-auto w-full max-w-5xl px-5 pb-9 pt-[38vh] sm:px-9 sm:pt-[30vh] lg:px-12">
        ...
        <h1 className="mt-5 text-5xl font-extrabold uppercase leading-[0.88] tracking-tight sm:text-6xl lg:text-7xl">
        ...
        <section className="mt-10 border-t border-white/10 pt-8" aria-labelledby="hunt-loop-title">
          ...
          <h2 className="mt-2 text-2xl font-bold" id="hunt-loop-title">One checkpoint. Three connected actions.</h2>
          ...
          <ol className="mt-6 grid gap-3 lg:grid-cols-3">
            <li className="rounded-xl border border-white/10 bg-black/45 p-5 backdrop-blur-sm">
              <div className="flex items-center justify-between"><span className="text-xs font-bold uppercase tracking-[0.16em] text-brand-400">1 · Personal</span><span aria-hidden="true" className="text-xl">◇</span></div>
              <h3 className="mt-4 text-lg font-bold">{hunt.sampleLoop.personalChallenge.title}</h3>
              <p className="mt-2 text-sm leading-6 text-white/60">{hunt.sampleLoop.personalChallenge.prompt}</p>
            </li>
            <li className="rounded-xl border border-white/10 bg-black/45 p-5 backdrop-blur-sm">
              <div className="flex items-center justify-between"><span className="text-xs font-bold uppercase tracking-[0.16em] text-cyan-300">2 · Team</span><span aria-hidden="true" className="text-xl">◎</span></div>
              <h3 className="mt-4 text-lg font-bold">Combine discoveries</h3>
              <p className="mt-2 text-sm leading-6 text-white/60">{hunt.sampleLoop.teamChallenge.prompt}</p>
              <p className="mt-3 rounded-lg bg-white/[0.06] px-3 py-2 font-sans text-xs tracking-wider text-white/65">{hunt.sampleLoop.teamChallenge.sharedInformation}</p>
            </li>
            <li className="rounded-xl border border-brand-400/25 bg-brand-400/[0.07] p-5 backdrop-blur-sm">
              <div className="flex items-center justify-between"><span className="text-xs font-bold uppercase tracking-[0.16em] text-brand-400">3 · Direction</span><span aria-hidden="true" className="text-xl">↗</span></div>
              <h3 className="mt-4 text-lg font-bold">Route unlocked: {hunt.sampleLoop.direction.destination}</h3>
        ...
        <div className="mt-8 rounded-xl border border-white/10 bg-black/55 p-4 backdrop-blur-sm sm:flex sm:items-center sm:justify-between sm:gap-6">
          ...
          <Link to={hunt.joinPath} className="mt-4 flex min-h-14 w-full shrink-0 items-center justify-center gap-3 rounded-md bg-brand-500 px-8 text-sm font-bold uppercase tracking-[0.13em] transition hover:bg-brand-400 sm:mt-0 sm:w-auto">Join this hunt <span aria-hidden="true">→</span></Link>
        </div>
